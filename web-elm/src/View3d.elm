module View3d exposing (view, update, Model(..), Msg(..), loadTexture, subscriptions, movementDecoder)
...
type Msg
    ...
    | ShiftLightAzimuth Float
    | ShiftLightElevation Float
      -- Depth scale
...
update msg model =
    case ( msg, model ) of
        ...
        ( ShiftLightAzimuth az, Rendering r ) ->
            ( Rendering { r | lighting = shiftLightAzimuth az r.lighting }, Cmd.none )

        ( ShiftLightElevation el, Rendering r ) ->
            ( Rendering { r | lighting = shiftLightElevation el r.lighting }, Cmd.none )
...
shiftLightAzimuth : Float -> Lighting -> Lighting
shiftLightAzimuth value lighting =
    let
        oldAzimuth: Float
        oldAzimuth = Angle.inDegrees lighting.azimuth
    in
    { lighting | azimuth = Angle.degrees (value + oldAzimuth) }


shiftLightElevation : Float -> Lighting -> Lighting
shiftLightElevation value lighting =
    let
        oldElevation: Float
        oldElevation = Angle.inDegrees lighting.elevation
    in
    { lighting | elevation = Angle.degrees (value + oldElevation) }
